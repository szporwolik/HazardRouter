...
func newManager(t *testing.T, reg *action.Registry, cfgs []config.Action) *action.Manager {
	t.Helper()
	m, err := action.NewManager(cfgs, reg, testLogger())
	if err != nil {
...
func TestSubmitUnknownTypeFailsConstruction(t *testing.T) {
	reg := action.NewRegistry()
	if _, err := action.NewManager([]config.Action{{ID: "x", Type: "nope", Enabled: true}}, reg, testLogger()); err == nil {
		t.Fatal("unknown type accepted")
	}
	// Even disabled entries with unknown types fail (config typo safety).
	if _, err := action.NewManager([]config.Action{{ID: "x", Type: "nope", Enabled: false}}, reg, testLogger()); err == nil {
